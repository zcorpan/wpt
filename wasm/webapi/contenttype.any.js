...
for (const contenttype of contenttypes) {
  promise_test(t => {
    ...
    return promise_rejects(t, new TypeError(), WebAssembly.compileStreaming(response).then(console.log) );
  }, `Response with Content-Type ${format_value(contenttype)}: compileStreaming`);

  promise_test(t => {
    const response = fetch(`/wasm/incrementer.wasm?pipe=header(Content-Type,${encodeURIComponent(contenttype)})`);
    return promise_rejects(t, new TypeError(), WebAssembly.instantiateStreaming(response).then(console.log) );
