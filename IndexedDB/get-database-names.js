async_test( async function(t) {
  let made_database_check = t.step_func(async function() {
    ...
    assert_true(idb_databases_promise.some(e => e.name == "TestDatabase" && e.version == 1), "Did not find database");
    t.done();
  });
...
async_test( function(t) {
  let done_making_databases_callback = t.step_func(async function() {
    let idb_databases_promise = await indexedDB.databases();
    assert_true(idb_databases_promise.some(e => e.name == "TestDatabase1" && e.version == 1), "Did not find database");
    assert_true(idb_databases_promise.some(e => e.name == "TestDatabase2" && e.version == 1), "Did not find database");
    assert_true(idb_databases_promise.some(e => e.name == "TestDatabase3" && e.version == 1), "Did not find database");
    t.done();
  });
...
async_test( function(t) {
  ...
  delete_request.onsuccess = t.step_func(async function() {
    let idb_databases_promise = await indexedDB.databases();
    assert_false(idb_databases_promise.some(e => e.name == "NonExistentDatabase"), "found excluded database");
    t.done();
  });
