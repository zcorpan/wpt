...
def _maybe_decode(s):
    """Decodes a binary-type string into text using ascii.

    Returns `unicode` in Python 2 and `str` in Python 3. The function is a
    no-op if the argument already has a text type.
    """
...
class RequestHeaders(dict):
    ...
    ensures all keys (i.e. names of headers) have text type while all values
    have binary type.
    """
    def __init__(self, items):
        for header in items.keys():
            key = _maybe_decode(header).lower()
...
class Authentication(object):
    ...
    def __init__(self, headers):
        ...
        if "authorization" in headers:
            ...
            auth_type = _maybe_decode(auth_type)
